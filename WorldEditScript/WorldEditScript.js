...
WorldEdit.prototype = {
	...
	buildMenu: function() {
		...
		//에딧메뉴 목록
		mm_edit.addMenu(this.contentType.RUN_FUNCTION, "Restore", function() {
		...
		mm_edit.addMenu(this.contentType.REDIRECT_MENU, "Copy", mme_copy);
		//원형 생성 에딧메뉴 목록
...
function we_initEdit(safeMode, workType, editor, editType, editDetail) {
	...
	switch(editType) {
		...
		if(editDetail === undefined) {//추가 정보가 없으면(로컬)
			...
			lo.addView(b6);
			sv.addView(lo);
			var dl2 = new we_dialog("Select 'Direction'", sv, null, null, "Cancel", function() {this.close()}, Gravity.CENTER);
		...
		if(editDetail === undefined) {//추가 정보가 없으면(로컬)
			...
			lo2.addView(b9);
			sv2.addView(lo2);
			var dl3 =  new we_dialog("Select 'Axis'", sv2, null, null, "Cancel", function() {this.close()}, Gravity.CENTER);
...
//잡일 처리를 gui로 하기위한 틀
function we_dialog(title, layout, confirmText, confirmFunc, cancelTxt, cancelFunc, gravity) {
	var that = this;
	this.name = title;
...
we_dialog.prototype = {
		...
		build: function() {
			...
			this.wd = new PopupWindow(rl, sg.wc, sg.wc, true);
		},
